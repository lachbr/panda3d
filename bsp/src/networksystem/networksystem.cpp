...
void NetworkCallbacks::OnSteamNetConnectionStatusChanged(SteamNetConnectionStatusChangedCallback_t *pCallback) {
  ...
  if (m_pPyCallback) {
    PyObject *pArgs = PyTuple_Pack(3,
                                   ...
                                   PyLong_FromSize_t(pCallback->m_eOldState));
...
void NetworkSystem::close_connection(NetworkConnectionHandle hConn) {
  ...
  if (m_bIsClient && hConn == m_hClientConnection) {
    m_hClientConnection = 0;
  }
}
...
void NetworkSystem::send_datagram(const Datagram &dg, NetworkSystem::NetworkSendFlags flags) {
  send_datagram(m_hClientConnection, dg, flags);
}
