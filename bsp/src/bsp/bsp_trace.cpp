                             ...
                             const LVector3 &v_point, vec_t epsilon = 0.0)
{
        ...
        for (x = 0; x < numpoints; x++)
        {
                ...
                if (dist < 0.0 && (epsilon == 0.0 || dist * dist > epsilon * epsilon * v_normal.dot(v_normal)))
                {
                        return false;
                }
        }
...
FaceFinder::FaceFinder(bspdata_t *data) : BaseBSPEnumerator(data)
{
}
...
void BSPTrace::add_dmodel(const dmodel_t *model, unsigned int mask)
{
        ...
        for (int facenum = 0; facenum < model->numfaces; facenum++)
        {
                ...
                PT(RayTraceTriangleMesh)
                geom = new RayTraceTriangleMesh;
