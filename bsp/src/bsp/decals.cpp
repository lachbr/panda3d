...
void R_DecalSurface(const dface_t *face, decalinfo_t *pinfo)
{
    ...
    PT(GeomTriangles)
    tris = new GeomTriangles(GeomEnums::UH_static);
                               ...
                               const int flags)
{
    ...
    NodePath hitbox;
    {
        ...
        if (merged_modelnum != 0 && !is_studio)
        {
            ...
            CPT(TransformState)
            ts = mdata.model_root.get_net_transform();
            ...
            CPT(TransformState)
            delta_ts = TransformState::make_pos_quat_scale(delta_origin, delta_quat, delta_scale);
    ...
    CPT(RenderAttrib)
    bma = BSPMaterialAttrib::make(mat);
    ...
    static const int num_attribs = 8;
    const RenderAttrib *attribs[num_attribs] = {
    ...
    CPT(RenderState)
    decal_state = RenderState::make(attribs, num_attribs, 1);
    ...
    if (info.lightmap)
    {
        ...
        //else
        //{
        lightmap_tex_attr = DCAST(TextureAttrib, lightmap_tex_attr)->add_on_stage(TextureStages::get_lightmap(), lm_tex);
        //}
    ...
    PT(GeomNode)
    decal_geomnode = new GeomNode("decal");
    ...
    PT(BoundingBox)
    bounds = new BoundingBox(mins, maxs);
