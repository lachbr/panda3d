...
static PStatCollector
    finddata_collector("AmbientProbes:UpdateNodes:FindNodeData");
static PStatCollector
    update_ac_collector("AmbientProbes:UpdateNodes:UpdateAmbientCube");
static PStatCollector
    update_locallights_collector("AmbientProbes:UpdateNodes:UpdateLocalLights");
static PStatCollector
    interp_ac_collector("AmbientProbes:UpdateNodes:InterpAmbientCube");
static PStatCollector
    copystate_collector("AmbientProbes:UpdateNodes:CopyState");
static PStatCollector
    addlights_collector("AmbientProbes:UpdateNodes:AddLights");
static PStatCollector
    fadelights_collector("AmbientProbes:UpdateNodes:FadeLights");
static PStatCollector
    ambientboost_collector("AmbientProbes:UpdateNodes:BoostAmbient");
static PStatCollector xformlight_collector("AmbientProbes:XformLight");
static PStatCollector
    loadcubemap_collector("AmbientProbes:UpdateNodes:LoadCubemap");
static PStatCollector
    findcubemap_collector("AmbientProbes:UpdateNodes:FindCubemap");

static ConfigVariableBool
    cfg_lightaverage("light-average", true,
...
static ConfigVariableDouble
    r_ambientmin("r_ambientmin", 0.3,
                 "Threshold above which ambient cube will not boost (i.e. it's "
                 "already sufficiently bright).");
static ConfigVariableDouble
    r_ambientfraction("r_ambientfraction", 0.1,
                      "Cheat. Fraction of direct lighting that ambient cube "
                      "must be below to trigger boosting.");
static ConfigVariableDouble
    r_ambientfactor("r_ambientfactor", 5.0,
...
AmbientProbeManager::
AmbientProbeManager() :
    _level(nullptr), _sunlight(nullptr),
...
AmbientProbeManager::AmbientProbeManager(BSPLevel *loader) :
    _level(loader), _sunlight(nullptr),
    //_light_kdtree( nullptr ),
    //_probe_kdtree( nullptr ),
    _envmap_kdtree(nullptr) {
}
...
void AmbientProbeManager::process_ambient_probes() {
  ...
  if (!_vis_root.is_empty()) { _vis_root.remove_node(); }
  _vis_root = _level->_result.attach_new_node("visAmbProbesRoot");
#endif

  bspdata_t *bspdata = _level->get_bspdata();

  _light_pvs.clear();
  _light_pvs.resize(bspdata->dmodels[0].visleafs + 1);
  _all_lights.clear();
  ...
  for (int entnum = 0; entnum < bspdata->numentities; entnum++) {
    ...
    if (!strncmp(classname, "light", 5)) {
      ...
      light->leaf = _level->find_leaf(light->pos);
      light->color =
          color_from_value(ValueForKey(ent, "_light"), true, true).get_xyz();
      light->type = lighttype_from_classname(classname);

      lightfalloffparams_t params = GetLightFalloffParams(ent, light->color);

      if (light->type == LIGHTTYPE_SUN || light->type == LIGHTTYPE_SPOT) {
        vec3_t angles;
      ...
      if (light->type == LIGHTTYPE_SPOT || light->type == LIGHTTYPE_POINT) {
        light->falloff = LVector4(params.constant_atten, params.linear_atten,
                                  params.quadratic_atten, 0);
        light->falloff2 =
            LVector4(params.start_fade_distance, params.end_fade_distance,
                     params.cap_distance, 0);
        light->falloff3 = LVector4(0, 0, 0, 0);
        if (light->type == LIGHTTYPE_SPOT) {
          float stopdot = FloatForKey(ent, "_inner_cone");
          if (!stopdot) { stopdot = 10; }
          float stopdot2 = FloatForKey(ent, "_cone");
          if (!stopdot2 || stopdot2 < stopdot) { stopdot2 = stopdot; }
          if (stopdot2 < stopdot) { stopdot2 = stopdot; }
      ...
      //        light_points.push_back( { light->pos[0], light->pos[1],
      //        light->pos[2] } );
      //}
    }
  }
  ...
  for (size_t lightnum = 0; lightnum < _all_lights.size(); lightnum++) {
    ...
    for (int leafnum = 0; leafnum < bspdata->dmodels[0].visleafs + 1;
         leafnum++) {
      if (light->type != LIGHTTYPE_SUN &&
          _level->is_cluster_visible(light->leaf, leafnum)) {
        _light_pvs[leafnum].push_back(light);
      }
    }
  }

  for (size_t i = 0; i < bspdata->leafambientindex.size(); i++) {
    dleafambientindex_t *ambidx = &bspdata->leafambientindex[i];
    ...
    for (int j = 0; j < ambidx->num_ambient_samples; j++) {
      dleafambientlighting_t *light =
          &bspdata
               ->leafambientlighting[ambidx->first_ambient_sample + j];
      PT(ambientprobe_t)
      probe = new ambientprobe_t;
      probe->leaf = i;
      probe->pos = LPoint3(
          RemapValClamped(light->x, 0, 255, leaf->mins[0], leaf->maxs[0]),
      ...
      for (int k = 0; k < 6; k++) {
        probe->cube.set_element(
            k, color_shift_pixel(&light->cube.color[k], 2.2));
      }
#ifdef VISUALIZE_AMBPROBES
      PT(TextNode)
      tn = new TextNode("visText");
      char text[40];
    ...
    if (probe_points.size()) { _probe_kdtrees[i]->build(probe_points); }
  }
}
...
void AmbientProbeManager::load_cubemaps() {
  ...
  std::cout << bspdata->cubemaps.size() << " cubemaps " << std::endl;
  _envmap_kdtree = new KDTree(3);
  vector<vector<double> > envmap_points;
  for (size_t i = 0; i < bspdata->cubemaps.size(); i++) {
    dcubemap_t *dcm = &bspdata->cubemaps[i];
    PT(cubemap_t)
    cm = new cubemap_t;
    cm->pos =
        LVector3(dcm->pos[0] / 16.0, dcm->pos[1] / 16.0, dcm->pos[2] / 16.0);
    ...
    cm->leaf = _level->find_leaf(cm->pos);
    cm->size = dcm->size;
    ...
    cm->cubemap_tex->setup_cube_map(dcm->size, Texture::T_unsigned_short,
                                    Texture::F_rgb);
    ...
    for (int j = 0; j < 6; j++) // for each side in the cubemap_tex
    {
      ...
      for (int y = 0; y < dcm->size; y++) {
        for (int x = 0; x < dcm->size; x++) {
          colorrgbexp32_t *col =
              &bspdata->cubemapdata[dcm->imgofs[j] + xel];
          LVector3 vcol;
  ...
  if (envmap_points.size()) { _envmap_kdtree->build(envmap_points); }
}

INLINE bool AmbientProbeManager::is_sky_visible(const LPoint3 &point) {
  if (_sunlight == nullptr) { return false; }
  ...
  Trace trace;
  CM_BoxTrace(ray, 0, CONTENTS_SKY | CONTENTS_SOLID, false, _level->get_colldata(),
              trace);

  return trace.has_hit() && trace.hit_contents == CONTENTS_SKY;
}

INLINE bool AmbientProbeManager::is_light_visible(const LPoint3 &point,
                                                  const light_t *light) {
  Ray ray((point + LPoint3(0, 0, 0.05)) * 16, light->pos * 16, LPoint3::zero(),
          LPoint3::zero());
  Trace trace;
  CM_BoxTrace(ray, 0, CONTENTS_SOLID, false, _level->get_colldata(), trace);
...
INLINE void apply_lightdatas(int n, const light_t *light,
                             CNodeShaderInput *input, const LMatrix4 &data,
                             bool active) {
  input->light_ids[n] = light->id;
  input->light_type[n] = light->type;
  input->light_data[n] = data;
  input->light_data2[n] = pack_lightdata2(light);
  if (active) input->active_lights++;
}
...
const RenderState *AmbientProbeManager::update_node(PandaNode *node,
  ...
  if (!node || !curr_trans) { return nullptr; }
  ...
  BSPLoader *loader = _level->get_loader();

  finddata_collector.start();
  bool new_instance = false;

  PT(CNodeShaderInput)
  input = DCAST(CNodeShaderInput, node->get_user_data());
  if (!input) {
    ...
    input->level_context = loader->get_level_context();
  ...
  CPT(TransformState)
  prev_trans = input->last_transform;
  ...
  bool pos_changed =
      pos_delta.length_squared() >= EQUAL_EPSILON || new_instance;

  bool average_lighting = cfg_lightaverage.get_value();

  bool lighting_current = loader->is_valid_level_context(input->level_context);
  if (!lighting_current) {
  ...
  int leaf_id = _level->find_leaf(curr_net);

  if (pos_changed) {
    // Update ambient cube
    if (_probes[leaf_id].size() > 0) {
      update_ac_collector.start();
      ambientprobe_t *sample = find_closest_in_kdtree(
          get_probe_kdtree(leaf_id), curr_net, _probes[leaf_id]);
    ...
    if (_cubemaps.size() > 0) {
      ...
      cubemap_t *cm =
          find_closest_in_kdtree(_envmap_kdtree, curr_net, _cubemaps);
      findcubemap_collector.stop();
      if (cm && cm->has_full_cubemap && cm != input->cubemap) {
        loadcubemap_collector.start();
        input->cubemap = cm;
        // Cubemap is in linear space
        input->cubemap_tex->setup_cube_map(cm->size, Texture::T_unsigned_short,
                                           Texture::F_rgb);
        input->cubemap_tex->set_ram_image(
            cm->cubemap_tex->get_ram_image(),
  ...
  if (pos_changed) {
    ...
    // Sort local lights from closest to furthest distance from node, we will
    // choose the two closest lights.
    std::sort(input->locallights.begin(), input->locallights.end(),
              [curr_net](const light_t *a, const light_t *b) {
                return (a->pos - curr_net).length_squared() <
                       (b->pos - curr_net).length_squared();
              });
  ...
  if (average_lighting) {
    ...
    memcpy(old_lightids, input->light_ids.p(),
           sizeof(int) * input->light_ids.size());
    memcpy(old_lightdata, input->light_data.p(),
           sizeof(LMatrix4) * input->light_data.size());
  ...
  if (average_lighting) {
    ...
    for (size_t i = 0; i < old_lightcount[0]; i++) {
      ...
      if (color.length_squared() < 1.0) { continue; }

      if (lights_updated >= MAX_TOTAL_LIGHTS) { break; }
    ...
    if (pos_changed || ambientcube_changed) {
      ...
      for (int i = 0; i < lights_updated; i++) {
        ...
        LVector3 lvec = (light->pos - curr_net) *
                        16.0; // lighting falloff parameters in hammer space
        float d2 = lvec.dot(lvec);
        float d = std::sqrt(d2);
        float atten = 1.0;

        float denom =
            light->falloff[0] + light->falloff[1] * d + light->falloff[2] * d2;
        if (denom > 0.00001) { atten = 1.0 / denom; }
      ...
      if (avg_cube_luminance < r_ambientmin &&
        ...
        float boost_factor =
            std::min((direct_light * r_ambientfraction.get_value()) / max_cube_luminance,
                     r_ambientfactor.get_value());
        for (int i = 0; i < 6; i++) {
          input->boxcolor_boosted[i] =
              input->boxcolor[i] * boost_factor; // boost
        }
  ...
  if (input->ambient_boost) {
    memcpy(input->ambient_cube.p(), input->boxcolor_boosted,
           sizeof(LVector3) * 6); // boost
  } else {
    memcpy(input->ambient_cube.p(), input->boxcolor,
           sizeof(LVector3) * 6); // no boost
  }
...
INLINE void xform_light(light_t *light, const LMatrix4 &cam_mat) {
  ...
  if (light->type !=
      LIGHTTYPE_POINT) // point lights have no directional component
  {
...
void AmbientProbeManager::xform_lights(const TransformState *cam_trans) {
  ...
  int cam_leaf = _level->find_leaf(cam_trans->get_pos());
