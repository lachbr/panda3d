...
float ImagePacker::
get_efficiency() {
  return (float)_area_used / (float)(_max_width * CeilPow2(_minimum_height));
}

bool ImagePacker::
reset(int sort_id, int max_width, int max_height) {
  int i;
...
get_minimum_dimensions(int *return_width, int *return_height) {
  *return_width = CeilPow2(_minimum_width);
  *return_height = CeilPow2(_minimum_height);
}
